...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    val nonCancelledResponse = response.takeWhile((A) => token.nonCancelled)
    for (s <- nonCancelledResponse) {exchange.write(s)}
    exchange.close()
  }
    ...
    val x = Future.run(){ ct =>
      Future {
        while (ct.nonCancelled) {
          ...
          request onComplete {
            case Success((request, exchange)) => respond(exchange, ct, handler(request))
            case Failure(t) => println("An error has occured: " + t.getMessage)
          }
        }
      }
    }
    Subscription(listenerSubscription, x)
  }
