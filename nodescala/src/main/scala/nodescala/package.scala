...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs match {
        case Nil => Future(Nil)
        case x :: xs  => x flatMap( a => all(xs).flatMap(as => Future(a :: as) ) )
      }
    }
    ...
    def delay(t: Duration): Future[Unit] = Future {
      Try(Await.ready(never, t)).getOrElse(())
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = Try(Await.result(f, -1 second)) match {
      case Success(a) => a
     ...
     */
    def continueWith[S](cont: Future[T] => S): Future[S] = f map( (t: T) => cont(f))
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = f map((t: T) => cont(Try(t)))
